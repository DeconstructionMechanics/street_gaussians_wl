...
from grtr import GRTR as GaussianTracer
from grtr import GRTRSettings as GaussianTracerSettings
from lib.lidar.utils.general_utils import build_scaling_rotation
import trimesh
...
def render_lidar(
    ...
    gaussian_tracer = GaussianTracer()

    def get_boundings(alpha_min=0.01):
        icosahedron = trimesh.creation.icosahedron()
        unit_icosahedron_vertices = torch.from_numpy(icosahedron.vertices).float().cuda() * 1.2584 
        unit_icosahedron_faces = torch.from_numpy(icosahedron.faces).long().cuda()

        mu = pc.get_xyz
        opacity = pc.get_opacity
        
        L = build_scaling_rotation(pc.get_scaling, pc.get_rotation)
        
        vertices_b = (2 * (opacity/alpha_min).log()).sqrt()[:, None] * (unit_icosahedron_vertices[None] @ L.transpose(-1, -2)) + mu[:, None]
        faces_b = unit_icosahedron_faces[None] + torch.arange(mu.shape[0], device="cuda")[:, None, None] * 12
        return vertices_b.reshape(-1, 3), faces_b.reshape(-1, 3)
    
    def build_bvh():
        vertices_b, faces_b = get_boundings(alpha_min=aabb_scale)
        gaussian_tracer.build_BVH(vertices_b, faces_b)
        
    def update_bvh():
        vertices_b, faces_b = get_boundings(alpha_min=aabb_scale)
        gaussian_tracer.update_BVH(vertices_b, faces_b)
        
    def trace(rays_o, rays_d):
        tracer_settings = GaussianTracerSettings(launch_x=rays_o.shape[0],
                                                 launch_y=1,
                                                 launch_z=1,
                                                 alpha_min=aabb_scale,
                                                 transmittance_min=1e-3,
                                                 sh_degree=pc.max_sh_degree)

        SinvR = build_scaling_rotation(1 / pc.get_scaling, pc.get_rotation)
        means3D = pc.get_xyz
        lidar_feature_map = [False, False, False, True, True]
        shs = pc.get_features[:, :, lidar_feature_map]
        shs = torch.cat((shs, torch.zeros((shs.shape[0], shs.shape[1], 1), dtype=torch.float32, device='cuda')), dim=2)
        opacity = pc.get_opacity
        colors, depth, alpha = gaussian_tracer.trace(rays_o, rays_d, means3D, opacity, SinvR, shs, tracer_settings)
        return {
            "render": colors,
            "depth": depth,
            "alpha" : alpha,
    ...
    build_bvh()
    trace_result = trace(viewpoint_lidarcamera.get_ray_o(), viewpoint_lidarcamera.get_ray_d())
    
    result = {
        "depth": trace_result['depth'].reshape(-1),
        "intensity": trace_result['render'][:,0].reshape(-1),
        "raydrop": trace_result['render'][:,1].reshape(-1),
        "n_contribute": torch.zeros(trace_result['depth'].shape[0], dtype=torch.float32, device='cuda'),
        "weights": trace_result['alpha'].reshape(-1),
    }
