...
from grtr import GRTRSettings

class StreetGaussianRTRenderer():
    def __init__(
        self,         
    ):
        self.cfg = cfg.render
    ...
    def render_kernel(
        ...
        # print('bg_color.shape=', bg_color.shape)
        # print('alpha.shape=', alpha.shape)
        alpha = alpha.permute(0, 2, 1)
        render = render + bg_color * (1 - alpha) 

        rendered_color = render.reshape(viewpoint_camera.image_height, viewpoint_camera.image_width, 3).permute(2, 0, 1)
        rendered_depth = depth.reshape(viewpoint_camera.image_height, viewpoint_camera.image_width, 1).permute(2, 0, 1)
        rendered_opacity = alpha.reshape(viewpoint_camera.image_height, viewpoint_camera.image_width, 1).permute(2, 0, 1)
