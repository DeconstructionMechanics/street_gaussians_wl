...
void backward_shs_cuda(int32_t num_rays, int32_t D, int32_t M, int32_t G,
    ...
    thrust::for_each(thrust::device,
        ...
        for (int32_t iG = 0; iG < G; iG++){
            ...
            computeFeaturesFromSHBackward(gaussian_idx, sh_degree, M, (float3*)means3D, ray_o, shs, clamped, dL_dfeature, (float3*)grad_means_from_shs, (float2*)grad_shs_from_shs); // TODO clamped
        }
    }); 
}
